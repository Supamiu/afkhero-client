...
namespace DigitalRuby.RainMaker
{
    public class RainCollision : MonoBehaviour
    {
        ...
        private void Emit(ParticleSystem p, ref Vector3 pos)
        {
            ...
            while (count != 0)
            {
                var yVelocity = UnityEngine.Random.Range(1.0f, 3.0f);
                var zVelocity = UnityEngine.Random.Range(-2.0f, 2.0f);
                var xVelocity = UnityEngine.Random.Range(-2.0f, 2.0f);
                const float lifetime = 0.75f;// UnityEngine.Random.Range(0.25f, 0.75f);
                var size = UnityEngine.Random.Range(0.05f, 0.1f);
        ...
        private void OnParticleCollision(GameObject obj)
        {
            if (RainExplosion != null && RainParticleSystem != null)
            {
                ...
                var count = RainParticleSystem.GetCollisionEvents(obj, collisionEvents);
                for (var i = 0; i < count; i++)
                {
                    var evt = collisionEvents[i];
                    var pos = evt.intersection;

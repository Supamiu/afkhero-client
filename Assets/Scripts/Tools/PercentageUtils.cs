    ...
    public class PercentageUtils : Singleton<PercentageUtils>
	{
		...
		public T GetItemFromPonderables<T> (IEnumerable<T> items) where T : Ponderable
		{
		    ...
		    foreach (var item in enumerable) {
				if (item.GetWeight () <= 0) {
					Debug.LogWarning ("Item with ponderation <= 0 found in ponderated array : " + item);
				}
				total += item.GetWeight ();
			}
			var result = Random.Range (1, total + 1);
			foreach (var item in enumerable) {
		...
		{
			var position = Random.Range (0, items.Count - 1);
        ...
        {
            var position = Random.Range(0, items.Length - 1);
            return items[position];
        }
